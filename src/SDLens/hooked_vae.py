...
class HookedAbstractAutoencoderKL:
    parent_cls = None
    vae = None  
    
    def __init__(self, vae: parent_cls):
    ...
    def run_with_hooks(self, 
        ...
        **kwargs
        ...
        try:
            output = self.vae.decode(*args, **kwargs)
        finally:
            for hook in hooks:
                hook.remove()
    ...
    def run_with_hooks_and_cache(self,
        ...
        **kwargs
        ...
        hooks = [hook for hook in hooks if hook is not None]
        output = self.vae.decoder(*args, **kwargs)
        for hook in hooks:
            hook.remove()
    ...
    def __setattr__(self, name, value):
        return setattr(self.vae, name, value)
...
class HookedAutoencoderKL(HookedAbstractAutoencoderKL):
    parent_cls = AutoencoderKL
